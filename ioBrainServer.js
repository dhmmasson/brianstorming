...
module.exports = { server : function( io ) {
	...
	io.sockets.on('connection', function (socket) {
		...
		socket.on("create", function( data ){
			...
			brainstorming.save() ;
			socket.emit( "info", brainstorming.toInfo() ) ;
		...
		socket.on("message", function( data ) {
			...
			if( activeBrainstorming.startTime == -1 ) {
				...
				io.to( activeBrainstorming.id ).emit( "info", activeBrainstorming.toInfo() ) ;
			}
			io.to( activeBrainstorming.id ).emit( "messages", [message] ) ;

			if( data.reference ) {
				var link = new Link( data.reference, message.id ) ;
				Link.verifyLink( link, function(){
					activeBrainstorming.addLink( link )
					io.to( activeBrainstorming.id ).emit( "links", [link] ) ;
				}, function() {  
